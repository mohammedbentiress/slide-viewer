    ...
    <Switch>
      {isMobile
        ? (
          <Route exact
          path="/controller/:id"
          render={() => <Controller slides={state.slides} currentSlide={state.currentSlide} />}
        />
          )
        : (
      <Route exact path="/edit/:id" render={() => (
        <div className="cont h-screen .bg-gray-200 flex justify-center items-center sm:p-6">
          ...
          <div className="border-2 border-black">
            <SlideShow slides={state.slides} currentSlide={state.currentSlide}/>
            <AppToolbar slides={state.slides} currentSlide={state.currentSlide} />
          </div>
        </div>
      )} />
          )
      ...
      <Route exact path="/present/:id" render={
        () => (
          <Present slides={state.slides} currentSlide={state.currentSlide}/>
        )}/>
      <Route exact path="*">
      {isMobile
        ? (
